...
export function calculateDailyHoroscope(zodiac: ZodiacSign, date: Date): DailyHoroscope {
  ...
  const baseHealth = 60 + ((dayHash * 13) % 25)
  const baseRelationship = 55 + ((dayHash * 17) % 30)
  const baseStudy = 50 + ((dayHash * 19) % 35)
  ...
  const health = randomScore(baseHealth)
  const relationship = randomScore(baseRelationship)
  const study = randomScore(baseStudy)
